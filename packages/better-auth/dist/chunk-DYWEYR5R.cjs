...
var getSession = () => createAuthEndpoint(
  ...
  async (ctx) => {
    try {
      ...
      const sessionDataPayload = sessionDataCookie ? chunkME4Q5ZEC_cjs.safeJSONParse(binary.binary.decode(base64.base64.decode(sessionDataCookie))) : null;
...
async function createEmailVerificationToken(secret, email, updateTo, expiresIn = 3600) {
  const token = await chunkG2LZ73E2_cjs.signJWT(
    {
...
var signInSocial = createAuthEndpoint(
  "/sign-in/social",
  {
    ...
    body: zod.z.object({
       ...
       */
      provider: chunkJ7OQS4OO_cjs.SocialProviderListEnum,
      /**
  ...
  async (c) => {
    ...
    const { codeVerifier, state } = await chunk5E75URIA_cjs.generateState(c);
...
var callbackOAuth = createAuthEndpoint(
  ...
  async (c) => {
    ...
    const { codeVerifier, callbackURL, link, errorURL, newUserURL } = await chunk5E75URIA_cjs.parseState(c);
    let tokens;
    try {
...
var updateUser = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalFields = chunkME4Q5ZEC_cjs.parseUserInput(
      ctx.context.options,
      rest,
      "update"
    );
...
var deleteUser = createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    if (ctx.context.options.user.deleteUser?.sendDeleteAccountVerification) {
      const token = chunkG2LZ73E2_cjs.generateRandomString(32, "0-9", "a-z");
      await ctx.context.internalAdapter.createVerificationValue({
...
var signUpEmail = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalData = chunkME4Q5ZEC_cjs.parseUserInput(
      ctx.context.options,
      additionalFields
    );
    let createdUser;
...
var linkSocialAccount = createAuthEndpoint(
  "/link-social",
  {
    ...
    body: zod.z.object({
       ...
       * OAuth2 provider to use`
       */
      provider: zod.z.enum(chunkJ7OQS4OO_cjs.socialProviderList, {
...
async function onRequestRateLimit(req, ctx) {
  ...
  const key = chunkME4Q5ZEC_cjs.getIp(req, ctx.options) + path;
