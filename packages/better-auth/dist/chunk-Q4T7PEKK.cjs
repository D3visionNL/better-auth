...
async function generator(ctx, options) {
  const baseEndpoints = chunkDUA2LDZ3_cjs.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = chunkDUA2LDZ3_cjs.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
var openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: chunkDUA2LDZ3_cjs.createAuthEndpoint(
      ...
      openAPIReference: chunkDUA2LDZ3_cjs.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
