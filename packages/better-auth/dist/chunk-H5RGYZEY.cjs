...
async function generator(ctx, options) {
  const baseEndpoints = chunkDRGY4F3Z_cjs.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = chunkDRGY4F3Z_cjs.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
var openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: chunkDRGY4F3Z_cjs.createAuthEndpoint(
      ...
      openAPIReference: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
