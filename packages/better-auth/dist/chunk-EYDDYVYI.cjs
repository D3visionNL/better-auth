...
async function generator(ctx, options) {
  const baseEndpoints = chunkVADINYB6_cjs.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = chunkVADINYB6_cjs.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
var openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: chunkVADINYB6_cjs.createAuthEndpoint(
      ...
      openAPIReference: chunkVADINYB6_cjs.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
