...
var jwt = (options) => {
  return {
    id: "jwt",
    endpoints: {
      getJwks: chunkVADINYB6_cjs.createAuthEndpoint(
        "/jwks",
        {
          method: "GET",
          metadata: {
          ...
          if (keySets.length === 0) {
            ...
            let jwk2 = {
              ...
              privateKey: privateKeyEncryptionEnabled ? JSON.stringify(
                await chunkXJGHQ3F6_cjs.symmetricEncrypt({
      ...
      getToken: chunkVADINYB6_cjs.createAuthEndpoint(
        "/token",
        {
          method: "GET",
          requireHeaders: true,
        ...
        async (ctx) => {
          ...
          let privateWebKey = privateKeyEncryptionEnabled ? await chunkXJGHQ3F6_cjs.symmetricDecrypt({
            key: ctx.context.options.secret,
