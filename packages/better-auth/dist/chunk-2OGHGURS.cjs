...
var facebook = (options) => {
  return {
    ...
    async createAuthorizationURL({ state, scopes, redirectURI }) {
      ...
      options.scope && _scopes.push(...options.scope);
      return await chunkH2JFIDVT_cjs.createAuthorizationURL({
        id: "facebook",
        options,
...
var google = (options) => {
  return {
    ...
    async createAuthorizationURL({ state, scopes, codeVerifier, redirectURI }) {
      ...
      options.scope && _scopes.push(...options.scope);
      const url = await chunkH2JFIDVT_cjs.createAuthorizationURL({
        id: "google",
        options,
...
var dropbox = (options) => {
  ...
  return {
    ...
    validateAuthorizationCode: async ({ code, codeVerifier, redirectURI }) => {
      return await chunkH2JFIDVT_cjs.validateAuthorizationCode({
        code,
        codeVerifier,
        redirectURI: options.redirectURI || redirectURI,
        options,
...
var reddit = (options) => {
  return {
    ...
    validateAuthorizationCode: async ({ code, redirectURI }) => {
      ...
      return chunkH2JFIDVT_cjs.getOAuth2Tokens(data);
    },
    async getUserInfo(token) {
      if (options.getUserInfo) {
        return options.getUserInfo(token);
