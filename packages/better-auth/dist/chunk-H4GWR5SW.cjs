...
var admin = (options) => {
  ...
  const adminMiddleware = chunkDRGY4F3Z_cjs.createAuthMiddleware(async (ctx) => {
    const session = await chunkDRGY4F3Z_cjs.getSessionFromCtx(ctx);
    if (!session?.session) {
      throw new chunkDRGY4F3Z_cjs.APIError("UNAUTHORIZED");
    }
    const user = session.user;
    if (!user.role || (Array.isArray(opts.adminRole) ? !opts.adminRole.includes(user.role) : user.role !== opts.adminRole)) {
      throw new chunkDRGY4F3Z_cjs.APIError("FORBIDDEN", {
  ...
  return {
    ...
    endpoints: {
      setRole: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        "/admin/set-role",
        {
          method: "POST",
          body: zod.z.object({
      ...
      createUser: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        "/admin/create-user",
        {
          method: "POST",
          body: zod.z.object({
        ...
        async (ctx) => {
          ...
          if (existUser) {
            throw new chunkDRGY4F3Z_cjs.APIError("BAD_REQUEST", {
          ...
          if (!user) {
            throw new chunkDRGY4F3Z_cjs.APIError("INTERNAL_SERVER_ERROR", {
      ...
      listUsers: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        "/admin/list-users",
        {
          method: "GET",
          use: [adminMiddleware],
      ...
      listUserSessions: chunkDRGY4F3Z_cjs.createAuthEndpoint(
      ...
      unbanUser: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        "/admin/unban-user",
        {
          method: "POST",
          body: zod.z.object({
      ...
      banUser: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        "/admin/ban-user",
        {
          method: "POST",
          body: zod.z.object({
      ...
      impersonateUser: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          if (!targetUser) {
            throw new chunkDRGY4F3Z_cjs.APIError("NOT_FOUND", {
      ...
      stopImpersonating: chunkDRGY4F3Z_cjs.createAuthEndpoint(
      ...
      revokeUserSession: chunkDRGY4F3Z_cjs.createAuthEndpoint(
      ...
      revokeUserSessions: chunkDRGY4F3Z_cjs.createAuthEndpoint(
      ...
      removeUser: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        "/admin/remove-user",
        {
          method: "POST",
          body: zod.z.object({
