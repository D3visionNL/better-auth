  ...
  return {
    ...
    listSessions: async (userId) => {
      if (secondaryStorage) {
        ...
        const list = chunkWRPAFI4I_cjs.safeJSONParse(currentList) || [];
        const now = Date.now();
        ...
        for (const session of validSessions) {
          ...
          if (sessionStringified) {
            ...
            const parsedSession = chunkWRPAFI4I_cjs.parseSessionOutput(ctx.options, {
              ...s.session,
    ...
    createSession: async (userId, request, dontRememberMe, override) => {
      ...
      const data = {
        ipAddress: request ? chunkWRPAFI4I_cjs.getIp(request, ctx.options) || "" : "",
      ...
      const res = await createWithHooks(
        ...
        secondaryStorage ? {
          fn: async (sessionData) => {
            ...
            const now = Date.now();
            if (currentList) {
              list = chunkWRPAFI4I_cjs.safeJSONParse(currentList) || [];
    ...
    findSession: async (token) => {
      if (secondaryStorage) {
        ...
        if (sessionStringified) {
          const s = JSON.parse(sessionStringified);
          const parsedSession2 = chunkWRPAFI4I_cjs.parseSessionOutput(ctx.options, {
            ...s.session,
            expiresAt: new Date(s.session.expiresAt),
            createdAt: new Date(s.session.createdAt),
            updatedAt: new Date(s.session.updatedAt)
          });
          const parsedUser2 = chunkWRPAFI4I_cjs.parseUserOutput(ctx.options, {
            ...s.user,
      ...
      const parsedSession = chunkWRPAFI4I_cjs.parseSessionOutput(ctx.options, session);
      const parsedUser = chunkWRPAFI4I_cjs.parseUserOutput(ctx.options, user);
      if (secondaryStorage) {
        await secondaryStorage?.set(
          token,
          JSON.stringify({
          ...
          parsedSession.expiresAt ? Math.floor(
            ((parsedSession.expiresAt instanceof Date ? parsedSession.expiresAt.getTime() : new Date(parsedSession.expiresAt).getTime()) - Date.now()) / 1e3
          ) : sessionExpiration
        );
      }
      return {
    ...
    deleteSessions: async (userIdOrSessionTokens) => {
      if (secondaryStorage) {
        if (typeof userIdOrSessionTokens === "string") {
          ...
          const sessions = activeSession ? chunkWRPAFI4I_cjs.safeJSONParse(activeSession) : [];
          if (!sessions) return;
...
var createTransform2 = (db, options, config) => {
  ...
  function transformValueToDB(value, model, field) {
    ...
    const f = schema[model].fields[field];
    if (f.type === "boolean" && type === "sqlite" && value !== null && value !== void 0) {
      return value ? 1 : 0;
    }
