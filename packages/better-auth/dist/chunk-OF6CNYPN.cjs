...
var verifyTwoFactorMiddleware = chunkVADINYB6_cjs.createAuthMiddleware(
  {
    body: zod.z.object({
      /**
...
function generateBackupCodesFn(options) {
  return Array.from({ length: 10 }).fill(null).map(() => chunkXJGHQ3F6_cjs.generateRandomString(10, "a-z", "0-9", "A-Z")).map((code) => `${code.slice(0, 5)}-${code.slice(5)}`);
}
async function generateBackupCodes(secret, options) {
  ...
  const encCodes = await chunkXJGHQ3F6_cjs.symmetricEncrypt({
...
async function getBackupCodes(backupCodes, key) {
  const secret = Buffer.from(
    await chunkXJGHQ3F6_cjs.symmetricDecrypt({ key, data: backupCodes })
  ).toString("utf-8");
...
var backupCode2fa = (options) => {
  ...
  return {
    id: "backup_code",
    endpoints: {
      verifyBackupCode: chunkVADINYB6_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const updatedBackupCodes = await chunkXJGHQ3F6_cjs.symmetricEncrypt({
      ...
      generateBackupCodes: chunkVADINYB6_cjs.createAuthEndpoint(
      ...
      viewBackupCodes: chunkVADINYB6_cjs.createAuthEndpoint(
...
var otp2fa = (options) => {
  ...
  const twoFactorTable = "twoFactor";
  const send2FaOTP = chunkVADINYB6_cjs.createAuthEndpoint(
    "/two-factor/send-otp",
    {
      method: "POST",
      use: [verifyTwoFactorMiddleware],
    ...
    async (ctx) => {
      ...
      const code = chunkXJGHQ3F6_cjs.generateRandomString(opts.digits, "0-9");
  ...
  const verifyOTP = chunkVADINYB6_cjs.createAuthEndpoint(
...
var totp2fa = (options) => {
  ...
  const twoFactorTable = "twoFactor";
  const generateTOTP = chunkVADINYB6_cjs.createAuthEndpoint(
    "/totp/generate",
    {
      method: "POST",
      use: [chunkVADINYB6_cjs.sessionMiddleware],
  ...
  const getTOTPURI = chunkVADINYB6_cjs.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const secret = await chunkXJGHQ3F6_cjs.symmetricDecrypt({
        key: ctx.context.secret,
  ...
  const verifyTOTP = chunkVADINYB6_cjs.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const decrypted = await chunkXJGHQ3F6_cjs.symmetricDecrypt({
        key: ctx.context.secret,
        data: twoFactor2.secret
      });
...
var twoFactor = (options) => {
  ...
  return {
    ...
    endpoints: {
      ...
      enableTwoFactor: chunkVADINYB6_cjs.createAuthEndpoint(
        "/two-factor/enable",
        {
          method: "POST",
          body: zod.z.object({
        ...
        async (ctx) => {
          ...
          if (!isPasswordValid) {
            throw new betterCall.APIError("BAD_REQUEST", {
              message: chunkVADINYB6_cjs.BASE_ERROR_CODES.INVALID_PASSWORD
            });
          }
          const secret = chunkXJGHQ3F6_cjs.generateRandomString(32);
          const encryptedSecret = await chunkXJGHQ3F6_cjs.symmetricEncrypt({
            key: ctx.context.secret,
            data: secret
          });
          const backupCodes = await generateBackupCodes(
      ...
      disableTwoFactor: chunkVADINYB6_cjs.createAuthEndpoint(
        "/two-factor/disable",
        {
          method: "POST",
          body: zod.z.object({
