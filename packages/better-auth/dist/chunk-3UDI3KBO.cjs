...
async function generator(ctx, options) {
  const baseEndpoints = chunkK3D45DZU_cjs.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = chunkK3D45DZU_cjs.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
var openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: chunkK3D45DZU_cjs.createAuthEndpoint(
      ...
      openAPIReference: chunkK3D45DZU_cjs.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
