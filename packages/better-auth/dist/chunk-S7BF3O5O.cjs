...
async function generator(ctx, options) {
  const baseEndpoints = chunkDYWEYR5R_cjs.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = chunkDYWEYR5R_cjs.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
var openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: chunkDYWEYR5R_cjs.createAuthEndpoint(
      ...
      openAPIReference: chunkDYWEYR5R_cjs.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
