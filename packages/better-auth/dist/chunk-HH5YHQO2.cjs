  ...
  return {
    ...
    listSessions: async (userId) => {
      if (secondaryStorage) {
        ...
        const list = chunkME4Q5ZEC_cjs.safeJSONParse(currentList) || [];
        const now = Date.now();
        ...
        for (const session of validSessions) {
          ...
          if (sessionStringified) {
            ...
            const parsedSession = chunkME4Q5ZEC_cjs.parseSessionOutput(ctx.options, {
              ...s.session,
    ...
    createSession: async (userId, request, dontRememberMe, override) => {
      ...
      const data = {
        ipAddress: request ? chunkME4Q5ZEC_cjs.getIp(request, ctx.options) || "" : "",
      ...
      const res = await createWithHooks(
        ...
        secondaryStorage ? {
          fn: async (sessionData) => {
            ...
            const now = Date.now();
            if (currentList) {
              list = chunkME4Q5ZEC_cjs.safeJSONParse(currentList) || [];
    ...
    findSession: async (token) => {
      if (secondaryStorage) {
        ...
        const s = JSON.parse(sessionStringified);
        const parsedSession2 = chunkME4Q5ZEC_cjs.parseSessionOutput(ctx.options, {
          ...s.session,
          expiresAt: new Date(s.session.expiresAt),
          createdAt: new Date(s.session.createdAt),
          updatedAt: new Date(s.session.updatedAt)
        });
        const parsedUser2 = chunkME4Q5ZEC_cjs.parseUserOutput(ctx.options, {
          ...s.user,
      ...
      const parsedSession = chunkME4Q5ZEC_cjs.parseSessionOutput(ctx.options, session);
      const parsedUser = chunkME4Q5ZEC_cjs.parseUserOutput(ctx.options, user);
      return {
    ...
    deleteSessions: async (userIdOrSessionTokens) => {
      if (secondaryStorage) {
        if (typeof userIdOrSessionTokens === "string") {
          ...
          const sessions = activeSession ? chunkME4Q5ZEC_cjs.safeJSONParse(activeSession) : [];
          if (!sessions) return;
