...
var binary = require('@better-auth/utils/binary');
...
var createAuthMiddleware = betterCall.createMiddlewareCreator({
  use: [
    optionsMiddleware,
    /**
...
var createAuthEndpoint = betterCall.createEndpointCreator({
...
var originCheckMiddleware = createAuthMiddleware(async (ctx) => {
  ...
  const matchesPattern = (url, pattern) => {
    ...
    if (pattern.includes("*")) {
      return wildcardMatch(pattern)(chunkS5UORXJH_cjs.getHost(url));
    }
    const protocol = chunkS5UORXJH_cjs.getProtocol(url);
    return protocol === "http:" || protocol === "https:" || !protocol ? pattern === chunkS5UORXJH_cjs.getOrigin(url) : url.startsWith(pattern);
  };
...
var getSession = () => createAuthEndpoint(
  ...
  async (ctx) => {
    try {
      ...
      const sessionDataPayload = sessionDataCookie ? chunkWRPAFI4I_cjs.safeJSONParse(binary.binary.decode(base64.base64.decode(sessionDataCookie))) : null;
      ...
      if (shouldBeUpdated) {
        const updatedSession = await ctx.context.internalAdapter.updateSession(
          session.session.token,
          {
            expiresAt: chunk2HPSCSV7_cjs.getDate(ctx.context.sessionConfig.expiresIn, "sec")
          }
        );
        if (!updatedSession) {
          chunkOJX3P352_cjs.deleteSessionCookie(ctx);
      ...
      await chunkOJX3P352_cjs.setCookieCache(ctx, session);
      return ctx.json(
        session
      );
    } catch (error2) {
...
async function createEmailVerificationToken(secret, email, updateTo, expiresIn = 3600) {
  const token = await chunkXJGHQ3F6_cjs.signJWT(
    {
  ...
  async (ctx) => {
    ...
    if (ctx.context.options.emailVerification?.autoSignInAfterVerification) {
      ...
      if (!currentSession || currentSession.user.email !== parsed.email) {
        ...
        await chunkOJX3P352_cjs.setSessionCookie(ctx, { session, user: user.user });
      }
    }
    if (ctx.query.callbackURL) {
      throw ctx.redirect(ctx.query.callbackURL);
  ...
  if (dbUser) {
    ...
    if (!hasBeenLinked) {
      ...
      if (!isTrustedProvider && !userInfo.emailVerified || c.context.options.account?.accountLinking?.enabled === false) {
        if (chunkVXYIYABQ_cjs.isDevelopment) {
      ...
      try {
        ...
        chunkH74YRRNV_cjs.logger.error("Unable to link account", e);
        return {
...
var signInSocial = createAuthEndpoint(
  "/sign-in/social",
  {
    ...
    body: zod.z.object({
       ...
       */
      provider: chunk2OGHGURS_cjs.SocialProviderListEnum,
      /**
  ...
  async (c) => {
    ...
    if (c.body.idToken) {
      ...
      await chunkOJX3P352_cjs.setSessionCookie(c, data.data);
      return c.json({
        redirect: false,
    ...
    const { codeVerifier, state } = await chunkLB4ZM24Q_cjs.generateState(c);
...
var callbackOAuth = createAuthEndpoint(
  "/callback/:id",
  {
    ...
    metadata: chunkH74YRRNV_cjs.HIDE_METADATA
  },
  async (c) => {
    let queryOrBody;
    try {
    ...
    const { codeVerifier, callbackURL, link, errorURL, newUserURL } = await chunkLB4ZM24Q_cjs.parseState(c);
    let tokens;
    try {
    ...
    await chunkOJX3P352_cjs.setSessionCookie(c, {
      session,
      user
    });
    let toRedirectTo;
...
var forgetPassword = createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const expiresAt = chunk2HPSCSV7_cjs.getDate(
      ctx.context.options.emailAndPassword.resetPasswordTokenExpiresIn || defaultExpiresIn,
      "sec"
    );
    const verificationToken = chunkH74YRRNV_cjs.generateId(24);
...
var updateUser = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalFields = chunkWRPAFI4I_cjs.parseUserInput(
      ctx.context.options,
      rest,
      "update"
    );
...
var deleteUser = createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    if (ctx.context.options.user.deleteUser?.sendDeleteAccountVerification) {
      const token = chunkXJGHQ3F6_cjs.generateRandomString(32, "0-9", "a-z");
      await ctx.context.internalAdapter.createVerificationValue({
...
var error = createAuthEndpoint(
  "/error",
  {
    method: "GET",
    metadata: {
      ...chunkH74YRRNV_cjs.HIDE_METADATA,
      openapi: {
...
var signUpEmail = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalData = chunkWRPAFI4I_cjs.parseUserInput(
      ctx.context.options,
      additionalFields
    );
    let createdUser;
    ...
    if (!ctx.context.options.emailAndPassword.autoSignIn || ctx.context.options.emailAndPassword.requireEmailVerification) {
      return ctx.json({
        token: null,
        user: {
          id: createdUser.id,
...
var linkSocialAccount = createAuthEndpoint(
  "/link-social",
  {
    ...
    body: zod.z.object({
       ...
       * OAuth2 provider to use`
       */
      provider: zod.z.enum(chunk2OGHGURS_cjs.socialProviderList, {
  ...
  async (c) => {
    ...
    const state = await chunkLB4ZM24Q_cjs.generateState(c, {
      userId: session.user.id,
      email: session.user.email
    });
    const url = await provider.createAuthorizationURL({
...
async function onRequestRateLimit(req, ctx) {
  ...
  const key = chunkWRPAFI4I_cjs.getIp(req, ctx.options) + path;
    ...
    api[key] = async (context = {}) => {
      ...
      let internalCtx = {
        ...
        setCookie(key2, value, options2) {
          betterCall.setCookie(endpoint.headers, key2, value, options2);
        },
        getCookie(key2, prefix) {
          const header = context.headers;
          const cookieH = header?.get("cookie");
          const cookie = betterCall.getCookie(cookieH || "", key2, prefix);
          return cookie;
        },
        getSignedCookie(key2, secret, prefix) {
          ...
          const cookie = betterCall.getSignedCookie(header, secret, key2, prefix);
          return cookie;
        },
        async setSignedCookie(key2, value, secret, options2) {
          await betterCall.setSignedCookie(endpoint.headers, key2, value, secret, options2);
        },
        redirect(url) {
          endpoint.headers.set("Location", url);
          return new betterCall.APIError("FOUND");
        },
      ...
      try {
        ...
        if (e instanceof betterCall.APIError) {
          ...
          for (const hook of afterHooks || []) {
            ...
            if (match) {
              try {
                ...
                if (e2 instanceof betterCall.APIError) {
          ...
          if (internalContext.context.returned instanceof betterCall.APIError) {
            internalContext.context.returned.headers = endpoint.headers;
      ...
      if (response instanceof betterCall.APIError) {
...
var router = (ctx, options) => {
  ...
  return betterCall.createRouter(api, {
    ...
    onError(e) {
      ...
      const log = optLogLevel === "error" || optLogLevel === "warn" || optLogLevel === "debug" ? chunkH74YRRNV_cjs.logger : void 0;
...
Object.defineProperty(exports, "APIError", {
  enumerable: true,
  get: function () { return betterCall.APIError; }
});
