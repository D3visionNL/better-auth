...
async function generator(ctx, options) {
  const baseEndpoints = chunkNKDIPVEC_cjs.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = chunkNKDIPVEC_cjs.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
var openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: chunkNKDIPVEC_cjs.createAuthEndpoint(
      ...
      openAPIReference: chunkNKDIPVEC_cjs.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
