...
function createCookieGetter(options) {
  const secure = options.advanced?.useSecureCookies !== void 0 ? options.advanced?.useSecureCookies : options.baseURL !== void 0 ? options.baseURL.startsWith("https://") ? true : false : chunkVXYIYABQ_cjs.isProduction;
...
async function setCookieCache(ctx, session) {
  ...
  if (shouldStoreSessionDataInCookie) {
    const data = base64.base64Url.encode(
      JSON.stringify({
        session,
        expiresAt: chunk2HPSCSV7_cjs.getDate(
