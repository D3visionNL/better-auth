...
function generateOTP(size) {
  return chunkG2LZ73E2_cjs.generateRandomString(size, "0-9");
}
var phoneNumber = (options) => {
  ...
  return {
    id: "phone-number",
    endpoints: {
      signInPhoneNumber: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          if (!session) {
            ...
            throw new betterCall.APIError("UNAUTHORIZED", {
              message: chunkDRGY4F3Z_cjs.BASE_ERROR_CODES.FAILED_TO_CREATE_SESSION
            });
          }
      ...
      sendPhoneNumberOTP: chunkDRGY4F3Z_cjs.createAuthEndpoint(
      ...
      verifyPhoneNumber: chunkDRGY4F3Z_cjs.createAuthEndpoint(
        "/phone-number/verify",
        {
          method: "POST",
          body: zod.z.object({
        ...
        async (ctx) => {
            ...
            if (options?.signUpOnVerification) {
              ...
              if (!user) {
                throw new betterCall.APIError("INTERNAL_SERVER_ERROR", {
                  message: chunkDRGY4F3Z_cjs.BASE_ERROR_CODES.FAILED_TO_CREATE_USER
                });
              }
            } else {
          ...
          if (!user) {
            throw new betterCall.APIError("INTERNAL_SERVER_ERROR", {
              message: chunkDRGY4F3Z_cjs.BASE_ERROR_CODES.FAILED_TO_UPDATE_USER
            });
          }
